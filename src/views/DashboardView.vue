...
<template>
  <div>
    <div class="mb-6 md:flex md:items-center md:justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-neutral-800">Welcome, {{ userName }}</h1>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="card">
        ...
        <div class="space-y-4">
          <div v-if="workStatusStore.isCheckedIn" class="flex items-center text-secondary-700">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
            </svg>
            <div>
              <p class="text-sm text-neutral-600">Checked in at</p>
              <p class="font-semibold">{{ workStatusStore.formattedLastCheckIn }}</p>
            </div>
          </div>
          <div v-else-if="workStatusStore.formattedLastCheckOut" class="flex items-center text-neutral-700">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
            </svg>
            <div>
              <p class="text-sm text-neutral-600">Last checked out</p>
              <p class="font-semibold">{{ workStatusStore.formattedLastCheckOut }}</p>
            </div>
          </div>
      ...
      <div class="card">
        ...
        <div v-if="workLogsStore.weeklyReport">
          <div class="flex justify-between mb-2">
            ...
            <span class="font-semibold">{{ workLogsStore.weeklyReport.total_duration || '0:00' }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-neutral-600">Days worked</span>
            <span class="font-semibold">{{ workLogsStore.weeklyReport.total_days || 0 }}/5</span>
          </div>
            ...
            <div class="flex items-center">
              <div class="flex-1 bg-neutral-200 rounded-full h-2">
                <div 
                  ...
                  :style="`width: ${Math.min((workLogsStore.weeklyReport.total_days || 0) * 20, 100)}%`"
                ></div>
              </div>
              <span class="text-sm text-neutral-600 ml-2">{{ (workLogsStore.weeklyReport.total_days || 0) * 20 }}%</span>
            </div>
          </div>
        </div>
        <div v-else class="text-neutral-600">
      ...
      <div class="card">
        ...
        <div v-if="absencesStore.pendingRequests && absencesStore.pendingRequests.length > 0">
          <ul class="space-y-3">
            <li v-for="request in absencesStore.pendingRequests.slice(0, 3)" :key="request.id" class="border-b border-neutral-200 pb-3 last:border-0">
              <div class="flex justify-between">
                <span class="font-medium">{{ request.absenceType?.name || 'Absence' }}</span>
